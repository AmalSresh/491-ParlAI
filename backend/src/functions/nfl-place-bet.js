...
app.http("nfl-place-bet", {
  ...
  handler: async (request, context) => {
    try {
      ...
      const { playerName, statLabel, statValue, direction, stake } = body;

      if (!playerName || !statLabel || !statValue || !direction || !stake) {
        return { status: 400, jsonBody: { error: "Missing required fields" } };
      }

      if (stake <= 0) {
        return { status: 400, jsonBody: { error: "Stake must be greater than 0" } };
      }
      ...
      const leagueResult = await pool.request()
        .query(`SELECT id FROM leagues WHERE name = 'NFL'`);
      
      let leagueId;
      if (leagueResult.recordset.length === 0) {
        // Insert NFL league if it doesn't exist
        const insertLeague = await pool.request()
          .input("name", sql.NVarChar, "NFL")
          .input("country", sql.NVarChar, "USA")
          .query(`
      ...
      // Create a prop event for this bet
      const eventResult = await pool.request()
        .input("leagueId", sql.Int, leagueId)
        .input("label", sql.NVarChar, `${playerName} - ${statLabel}`)
        .query(`
      ...
      const marketResult = await pool.request()
        ...
        .input("type", sql.VarChar, "player_prop")
        .query(`
      ...
      // Create selection (the actual bet option)
      const odds = direction === "more" ? 1.9 : 1.9; // ~-110 American odds
      const selectionResult = await pool.request()
        .input("marketId", sql.BigInt, marketId)
        .input("label", sql.NVarChar, `${direction === "more" ? "↑ More" : "↓ Less"} ${statValue} ${statLabel}`)
        .input("odds", sql.Decimal(10, 4), odds)
        .input("lineValue", sql.Decimal(10, 2), statValue)
        .query(`
      ...
      await pool.request()
        ...
        .input("type",   sql.VarChar, "bet_placed")
        .query(`
      ...
      context.error("Error placing NFL bet:", error);
